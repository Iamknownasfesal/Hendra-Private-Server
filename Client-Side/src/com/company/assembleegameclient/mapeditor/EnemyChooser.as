...
class EnemyChooser extends Chooser
{
    ...
    function EnemyChooser(_arg1:String = "")
    {
    ...
    {
        var _local8:XML = null;
        var _local11:RegExp = null;
        var _local13:String = null;
        var _local14:int = 0;
        var _local15:ObjectElement = null;
        if(!_arg5)
        {
            removeElements();
        }
        ...
        for each(_local8 in _local12)
        {
            ...
            if(!(_local11 != null && _local13.search(_local11) < 0))
            {
                if(_arg2 != "")
                {
                    _local9 = Boolean(_local8.hasOwnProperty(_arg2))?Number(Number(_local8.elements(_arg2))):Number(-1);
                    if(_local9 < 0)
                    {
                        continue;
                    }
                    _local6 = _local9 < _arg3?Boolean(false):Boolean(true);
                    _local7 = _arg4 > 0 && _local9 > _arg4?Boolean(false):Boolean(true);
                }
                if(_local6 && _local7)
                {
