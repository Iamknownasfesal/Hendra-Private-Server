    ...
    partial class Player
    {
        ...
        public void UseItem(RealmTime time, int objId, int slot, Position pos)
        {
            using (TimedLock.Lock(_useLock))
            {
                ...
                if (item.ActivateEffects.Any(eff => eff.Effect == ActivateEffects.Backpack))
                {
                    if (HasBackpack == true)
                    {
            ...
            foreach (var eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.UnlockSlot:
                        UnlockSlot(time, item, target, eff);
                        break;
        ...
        private void AECommonCrate()
        {
            var gameData = Manager.Resources.GameData;

            List<String> itemList = new List<string>
            {
                "Ravenheart Sword", "Demon Edge",
                "Staff of Horror", "Golden Bow",
                "Fire Dagger", "Wand of Death",
                "Cloak of the Night Thief", "Elvencraft Quiver",
                "Scorching Blast Spell", "Tome of Rejuvenation",
                "Red Iron Helm", "Reinforced Shield",
                "Seal of the Aspirant", "Stingray Poison",
                "Soul Siphon Skull", "Savage Trap",
                "Neutralization Orb", "Hallucination Prism",
                "Arcblast Scepter", "Silver Star"
            };
            ...
            int pick = rand.Next(0, itemList.Count);

            objType = gameData.IdToObjectType[itemList[pick]];

            var Item = gameData.Items[objType];

            //itemChoosing
            var frislot = Inventory.GetAvailableInventorySlot(Item);

            if (frislot != -1)
            {
                Inventory[frislot] = Item;

                SendHelp($"Awesome you found {itemList[pick]}");
                foreach (var i in Owner.Players.Values)
                    SendInfo($"{Name} unboxed a {itemList[pick]} from a Common Crate!");
            }
        }
        ...
        private void UnlockSlot(RealmTime time, Item item, Position target, ActivateEffect eff)
        {
