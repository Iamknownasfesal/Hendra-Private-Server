...
namespace common
{
    public class Database : IDisposable
    {
        ...
        public Task<object> UpdateCurrency(DbAccount acc, CurrencyType fame, ITransaction trans)
        {
        ...
        public DbAccount CreateGuestAccount(string uuid)
        {
            ...
            var acnt = new DbAccount(_db, 0)
            {
                ...
                Prestige = newAccounts.Prestige,
                TotalPrestige = newAccounts.Prestige,
                PassResetToken = ""
            };
        ...
        private static readonly Dictionary<CurrencyType, string[]> CurrencyKey = new Dictionary<CurrencyType, string[]>
        {
            ...
            { CurrencyType.Prestige, new [] { "totalPrestige", "prestige" } }
        };
        ...
        public Task UpdatePrestige(DbAccount acc, int amount, ITransaction transaction = null)
        {
            var trans = transaction ?? _db.CreateTransaction();

            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "totalPrestige", amount)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.TotalPrestige = (int)t.Result;
                    });

            var task = trans.HashIncrementAsync(acc.Key, "prestige", amount)
                .ContinueWith(t =>
                {
                    if (!t.IsCanceled)
                        acc.Prestige = (int)t.Result;
                });
        ...
        public void WipeAccount(DbAccount acc, XmlData dat, string killer)
        {
            ...
            _db.HashSetAsync(acc.Key, "fame", 0, flags: CommandFlags.FireAndForget);
            _db.HashSetAsync(acc.Key, "totalFame", 0, flags: CommandFlags.FireAndForget);
            _db.HashSetAsync(acc.Key, "prestige", 0, flags: CommandFlags.FireAndForget);
            _db.HashSetAsync(acc.Key, "totalPrestige", 0, flags: CommandFlags.FireAndForget);
